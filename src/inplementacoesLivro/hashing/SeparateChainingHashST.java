...
public class SeparateChainingHashST<Key, Value> {
	...
	/*
	 * private void resize(int chains) { SeparateChainingHashST<Key, Value> temp =
	 * new SeparateChainingHashST<Key, Value>(chains); for (int i = 0; i < m; i++) {
	 * for (Key key : st[i].keys()) { temp.put(key, st[i].get(key)); } } this.m =
	 * temp.m; this.n = temp.n; this.st = temp.st; }
	 */
	 ...
	 */
	public boolean contains100(Key key) {
		if (key == null)
			throw new IllegalArgumentException("argument to contains() is null");
		return get100(key) != null;
	}
	 ...
	 * @throws IllegalArgumentException if {@code key} is {@code null}
	 */
	public void putHash100(Key key, Value val) {
		if (key == null)
	...
	public void putHash97(Key key, Value val) {
		if (key == null)
			throw new IllegalArgumentException("first argument to put() is null");
		if (val == null) {
			delete(key);
